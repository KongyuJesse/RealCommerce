...
function App() {
  ...
  const activeHeroSlide = marketplaceHomeData.heroSlides[activeHeroIndex];

  useEffect(() => {
    const intervalId = window.setInterval(() => {
      setActiveHeroIndex((current) => (current + 1) % marketplaceHomeData.heroSlides.length);
    }, 5200);
    ...
    <div className="amazon-shell">
      <header>
        <div className="navbar-top">
          <div className="nav-left">
            ...
            <div className="deliver-to">
              ...
              <div className="deliver-text">
                ...
                <span className="deliver-line2">{marketplaceHomeData.location}</span>
              </div>
            </div>
          </div>
          ...
          <div className="nav-right">
            <div className="lang-flag">
              <FlagUsaIcon size={18} />
              <span>{marketplaceHomeData.language}</span>
        ...
        <div className="nav-secondary">
          ...
          <div className="nav-options">
            {marketplaceHomeData.secondaryLinks.map((item) => (
              <span key={item}>{item}</span>
            ))}
          </div>

          <div className="nav-deals">{marketplaceHomeData.spotlightLabel}</div>
        </div>
      </header>

      <main className="main-content" id="shop">
        <section className="hero-section" aria-label="Featured promotions">
          {marketplaceHomeData.heroSlides.map((slide, index) => (
            <div
      ...
      <footer>
        <div className="footer-links">
          {marketplaceHomeData.footerColumns.map((column) => (
